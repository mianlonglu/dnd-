	...
	switch (pattern){
		case 1:{
	        ...
	        for (int i = 0; i < category; i++) {
	            ...
	            fgets(monsters[i].name, MAX_LENGTH, stdin);
                sscanf(monsters[i].name, "%s %d", monsters[i].name, &num);
	            clear_input_buffer();
	        ...
	        for (int k = 0; k < all_monsters_size - 1; k++) {
	            for (int l = 0; l < all_monsters_size - k - 1; l++) {
	        ...
	        // 输出排序后的所有怪物个体信息
	        printf("排序后的怪物信息:\n");
	        for (int i = 0; i < all_monsters_size; i++) {
		...
		case 2:
    	{
    		...
    		if (characters == NULL) {
				perror("Failed to allocate memory for characters");
				exit(EXIT_FAILURE);
			}

			for (int i = 0; i < num; i++) {
        		characters[i].names = (char *)malloc(MAX_LENGTH * sizeof(char));
				if (characters[i].names == NULL) {
					perror("Failed to allocate memory for characters[i].names");
					exit(EXIT_FAILURE);
				}
			}
    		...
    		for (int i = 0; i < num; i++) {
        		...
        		while (inputIndex < strlen(input) && input[inputIndex]!= '\n') {
            		int result = sscanf(&input[inputIndex], "%s %d", characters[i].names, &characters[i].dexterities);

            		if (result == 2) { // 如果成功读取了两个值（名称和敏捷值）
						// 更新输入索引，跳过已解析的部分
						consumedChars = strlen(characters[i].names) + 1 + /* Length of an integer value */;

						inputIndex += consumedChars;
